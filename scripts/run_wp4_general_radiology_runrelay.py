...
The task resolves the two legacy auxiliary input files on the bound workstation,
runs synthetic validation, extracts aggregate TriNetX utilization, then scales
observed strata to GBD prevalence. No patient-level artifact is written.
"""
...
def main() -> None:
    ...
    gbd_file = find_exact_file(GBD_NAME)

    resolved = {
        "trinetx_control_dir": str(CONTROL_DIR),
        "zip_map": str(zip_map),
        "zip_map_sha256": sha256_file(zip_map),
        "gbd_file": str(gbd_file),
        "gbd_file_sha256": sha256_file(gbd_file),
    }
    ...
    summary = {
        "status": "WP4_GENERAL_RADIOLOGY_RUNRELAY_OK",
    ...
    progress("complete", "WP4 corrected disease-based clinical-volume run complete")
