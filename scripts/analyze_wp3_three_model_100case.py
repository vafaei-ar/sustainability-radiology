...
def main() -> None:
    ...
    review_ids = []
    base, rem = divmod(REVIEW_N, len(strata))
    for sidx, key in enumerate(sorted(strata)):
        ids = list(strata[key])
        local = random.Random(f"{SEED}:{key}")
        local.shuffle(ids)
        review_ids.extend(ids[:base + (1 if sidx < rem else 0)])
    review_ids = sorted(review_ids)
    if len(review_ids) != REVIEW_N:
        raise RuntimeError(f"Expected {REVIEW_N} review cases, found {len(review_ids)}")
    ...
    summary = {"status": "WP3_THREE_MODEL_100CASE_ANALYSIS_OK", "seed": SEED, "bootstrap_iterations": BOOT, "models": ["MedGemma-4B", "Qwen2.5-VL-7B-Instruct", "InternVL3-8B"], "operational_pairwise": operational, "utility_pairwise": utility, "clinical_review_packet": {"n_cases": REVIEW_N, "selection": "Deterministic balanced sample across MeSH normal/non-normal metadata status and reference-length quartile.", "blinding": "Model identities randomized independently by case across output columns A/B/C. Unblinding key stored separately.", "adjudication_status": "Not performed. Blank fields are provided for human clinical review."}, "interpretation": {"energy_scope": "Direct NVIDIA GPU board operational energy; gross primary and idle-adjusted net secondary.", "operational_inference_unit": "Ten matched 10-case blocks.", "utility_inference_unit": "One hundred paired cases.", "utility_limit": "Unigram F1 and ROUGE-L are lexical screening metrics only and do not establish clinical adequacy.", "multiplicity": "Holm adjustment is reported separately within the nine operational pairwise sign tests and six utility pairwise sign tests."}}
    (OUT / "summary.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
    (OUT / "analysis_report.md").write_text("# WP3 three-model 100-case paired analysis\n\nOperational comparisons use ten matched 10-case blocks. Utility comparisons use 100 paired cases. Lexical metrics are screening endpoints only. A deterministic 50-case blinded clinical-review packet was prepared; no clinical adjudication was performed.\n", encoding="utf-8")
